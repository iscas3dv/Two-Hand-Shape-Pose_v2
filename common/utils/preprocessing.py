...
# We add function generate_mano_skeleton() which returns recorded MANO joint info.
# We modifiy the function augmentation().
...
def generate_mano_skeleton():
    ...
    skeleton = [{} for _ in range(JOINT_NUM * 2)]
    for hand_type in ("right","left"):
        for finger_id, finger_name in enumerate(finger_names):
            for finger_joint_id in range(1,5):
                joint_id = JOINT_LEVEL_IDX[hand_type][finger_joint_id][finger_id]
                joint_parent_id = JOINT_LEVEL_IDX[hand_type][finger_joint_id - 1][finger_id]
                skeleton[joint_id]['name'] = hand_type[0] + '_' + finger_name + str(finger_joint_id)
                skeleton[joint_id]['parent_id'] = joint_parent_id
    skeleton[0]['name']="r_wrist"
    skeleton[0]['parent_id']=-1
    skeleton[JOINT_NUM]['name']="l_wrist"
    skeleton[JOINT_NUM]['parent_id']=-1
    # save child_id
    for i in range(len(skeleton)):
        joint_child_id = []
        for j in range(len(skeleton)):
...
def augmentation(img, bbox, joint_coord, joint_valid, hand_type, mode, joint_type, shape_param, shape_type):
    img = img.copy(); 
    joint_coord = joint_coord.copy(); 
    hand_type = hand_type.copy();
    shape_param = shape_param.copy();
    ...
    if do_flip:
        ...
        shape_param[shape_type['right']], shape_param[shape_type['left']] = shape_param[shape_type['left']].copy(), shape_param[shape_type['right']].copy()
        hand_type[0], hand_type[1] = hand_type[1].copy(), hand_type[0].copy()
    rot_rad = np.pi * rot / 180
    sn, cs = np.sin(rot_rad), np.cos(rot_rad)
    global_rotation = np.array([[cs, sn],[-sn, cs]])
    joint_coord[:,:2] = joint_coord[:,:2] @ global_rotation.T
    return img, joint_coord, joint_valid, hand_type, inv_trans, shape_param
